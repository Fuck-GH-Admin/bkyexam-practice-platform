...
- 后端 command/query 已覆盖 Auth、Bank Mapping、Import Jobs dry-run、Question Review、System Status、Audit Logs。
- 已有 `npm run admin:bootstrap` 创建第一个 `super_admin`。
- 仍缺 Admin User manage、真正写入 import mode、import error report。
...
Admin Console
  ...
  Admin Users
    ...
    List / Create / Disable / Role Update (next)
```
...
| Admin Users | future read/manage under `admin_user:manage` | future create/disable/role update | super_admin |
...
- No HTTP Admin User manage API yet.

Next:

- Admin User list/detail/create/disable/role update.
- Must not expose public registration.
- Must audit all lifecycle changes.
...
- Admin User manage decision is made.
...
当前建议：继续做后端 B5.8，不启动正式 Admin 前端。
