...
- Practice/Wrongbook/Admin Auth/Admin Bank Mapping/Admin System Status/Admin Import Job/Admin Question Review v1 schema 的计数不变量、写入版本边界、导入任务 summary/error boundary、题目质检 flag/exclusion boundary、`false`、legacy UUID、角色/权限和 strict response boundary。
- session card/page contract 的来源、timestamp、计数和分页边界。

其中 shared 19 项、API 317 项、Web 31 项。Practice/Wrongbook/Admin route 还会故意注入不合法 repository payload，确认 runtime schema 不会把错误数据伪装成 `200`。
...
2. 等待数据库 healthcheck。
3. 对空的 `bkyexam_test` 执行全部七份 migration。
...
- Admin Auth/RBAC/session/audit foundation、Admin Bank Mapping list/detail/update/bulk-status、Admin System Status、Admin Import Jobs create/list/detail/audit/status summary、Admin Question Review flag/exclusion/status summary、version conflict、audit log，且 `bky_admin_session` 与 `bky_session` 隔离。
