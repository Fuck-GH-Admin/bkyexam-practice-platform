...
其中 shared 25 项、API 369 项、Web 31 项。Practice/Wrongbook/Learning/Admin route 还会故意注入不合法 repository payload，确认 runtime schema 不会把错误数据伪装成 `200`。
...
2. 等待数据库 healthcheck。
3. 对空的 `bkyexam_test` 执行全部九份 migration。
