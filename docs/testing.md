...
- Practice/Wrongbook/Admin Auth v1 schema 的计数不变量、`false`、legacy UUID、角色/权限和 strict response boundary。
- session card/page contract 的来源、timestamp、计数和分页边界。

其中 shared 14 项、API 273 项、Web 31 项。Practice/Wrongbook route 还会故意注入不合法 repository payload，确认 runtime schema 不会把错误数据伪装成 `200`。
...
- Admin Auth/RBAC/session/audit foundation，且 `bky_admin_session` 与 `bky_session` 隔离。
