...
Admin 后端 contract 已在 [`admin-backend-contract.md`](admin-backend-contract.md) 中完成设计，其中 Admin Auth/RBAC/Audit foundation、Bank Mapping read/write API、System Status API 与 Import Jobs dry-run API 已实现；Question Review 仍未实现 route/shared schema。详细规则见 [contracts.md](contracts.md)。
...
8. 管理平台已有后端 contract、Admin Auth/RBAC/Audit foundation、`/api/admin/auth/*`、题库 mapping read/write API、System Status API 与 Import Jobs dry-run API，但还没有独立应用或 Question Review API。
