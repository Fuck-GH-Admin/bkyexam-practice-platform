...
Admin 后端 contract 已在 [`admin-backend-contract.md`](admin-backend-contract.md) 中完成设计，其中 Admin Auth/RBAC/Audit foundation 已实现；Bank Mapping、Import Job、Question Review、System Status 仍未实现 route/shared schema。Import Job 仅在 Admin contract 中定义，readiness/DB health 尚未迁入 shared。详细规则见 [contracts.md](contracts.md)。
...
8. 管理平台已有后端 contract、Admin Auth/RBAC/Audit foundation 与 `/api/admin/auth/*`，但还没有独立应用和题库整理等管理业务 API。
