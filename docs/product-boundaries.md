...
- 账户说明、身份安全和数据归属提示。
- 学习统计后端概览已具备；仍缺前端呈现、学习目标和长期进步反馈。
...
当前学习概览通过 `LearningDashboardRepository` 从 Practice/Wrongbook 事实表聚合，不直接拥有 session 生命周期。
...
- Auth、Catalog、Learning、Admin Auth、Admin User manage、Admin Bank Mapping read/write、Admin System Status、Admin Import Job dry-run/Error Report/true import gate、Admin Question Review、Admin Audit Log、通用 error 已迁入 shared v1；reset import、异步队列和正式 Admin UI 尚未实现。
