...
- Auth、Catalog、Learning Dashboard/Trends/Goals/Review Marks、Admin Auth、Admin User manage、Admin Bank Mapping read/write、Admin System Status、Admin Import Job dry-run/Error Report/true import gate/reset/cancel/retry/worker heartbeat、Admin Question Review detail/override、Admin Audit Log、通用 error 已迁入 shared v1；durable import worker/heartbeat 已实现，实时 progress 事件流尚未实现。
