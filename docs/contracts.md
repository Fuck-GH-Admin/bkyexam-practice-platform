...
| Admin | Auth schemas, role/permission schemas, Bank Mapping read request/list/detail schemas |
...
- Admin Bank Mapping write、Question Review、System Status、Audit Log read API。后端 contract 已在 [`admin-backend-contract.md`](./admin-backend-contract.md) 中完成设计，尚未迁入 `packages/shared`。
...
- Admin Auth 与 Bank Mapping read shared Zod schema/route 已实现；Admin 其余 backend contract 尚未迁入 shared，也尚未实现 route。
