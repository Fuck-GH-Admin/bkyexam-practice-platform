...
| Admin | Auth schemas, role/permission schemas, Bank Mapping read/write request/list/detail/bulk-status schemas |
...
- Question Review、System Status、Audit Log read API。后端 contract 已在 [`admin-backend-contract.md`](./admin-backend-contract.md) 中完成设计，尚未迁入 `packages/shared`。
...
- Admin Auth 与 Bank Mapping read/write shared Zod schema/route 已实现；Admin 其余 backend contract 尚未迁入 shared，也尚未实现 route。
